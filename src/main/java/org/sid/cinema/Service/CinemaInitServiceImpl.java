...
public class CinemaInitServiceImpl implements ICinemaInitService {
	...
	public void initCinemas() {
		villeRepository.findAll().forEach(v -> {
			Stream.of("Mega rama", "iMax", "Founoun", "Chahrazad", "Daouliz").forEach(nameCinema -> {
				Cinema cinema = new Cinema();
				cinema.setName(nameCinema);
				cinema.setNombreSale(3 + (int) (Math.random() * 7));
				cinema.setVille(v);
	...
	public void initSalles() {
		cinemaRepository.findAll().forEach(cinema -> {
			for (int i = 0; i < cinema.getNombreSale(); i++) {
	...
	public void initCategories() {
		Stream.of("Histoire", "Action", "Fiction", "Drama", "Commedie").forEach(cat -> {
	...
	public void initFilms() {
		...
		Stream.of("12 hommes en colère", "Forrest gump", "Green book", "La ligne verte", "Le parrain",
				"Le seigneur des anneaux", "Gladiator", "Le terminal").forEach(titreFilm -> {
					Film film = new Film();
